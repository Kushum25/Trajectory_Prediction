...
#Step 9.1: Run tests for LINE
plt.plot(Y_test_pred,Y_test_err,'ro')
plt.xlabel('Predicted')
plt.ylabel('Residual')
plt.grid()

#Step 9.2: Obs-Predicted plot
...
#Step 9.3: Breusch Pagan Test for homoskedasticity
BP1 = sm.stats.diagnostic.het_breuschpagan(Y_test_err,X_test)

#Step 9.4: Tests of Normality
sm.qqplot(Y_test_err,line ='s')
plt.grid()

sm.stats.diagnostic.kstest_normal(Y_test_err, dist='norm')

#Step 9.5: Autocorrelation testing
sm.stats.diagnostic.acorr_breusch_godfrey(result) #need to provide regression model


# Step 10: Create a confusion Matrix
#Step 10.1:Normalize the data between 0 and 1 for Y_train and Y_predict
...
#Step 10.2: Reclassify the normalized value to binary
#for Training dataset Y_train
threshold = 0.5
...
# Step 10.3: Create a confusion Matrix
cnf_matrix = metrics.confusion_matrix(Y_REtest, Y_REpred2)
cnf_matrix # y_test is going be rows (obs), y_pred (predicted) are cols

# Step 10.4: Evaluate usng accuracy, precision, recall
...
# Step 10.5: Plot confusion Matrix
class_names=[0,1] # name  of classes
fig, ax = plt.subplots()
tick_marks = np.arange(len(class_names))
plt.xticks(tick_marks, class_names)
